...
function sourceMeta(src){return''}
function sourceBlock(sources){return''}
...
function renderShell(id,title,desc,sources,body){document.getElementById(id).innerHTML=`<div class="module-head"><div><h2>${esc(title)}</h2><p>${rich(cleanUiText(desc))}</p></div>${moduleControls(id)}</div><div class="module-layout">${moduleRail(id)}<div class="module-content">${activityHub(id)}<div class="stats"></div>${sourceBlock(sources)}${body}</div></div>`;renderStats();applyMode();}
...
function drawDisease(){const q=(diseaseSearch?.value||'').toLowerCase(),g=diseaseGroup?.value||'';diseaseRows.innerHTML=DATA.diseases.filter(d=>(!g||d.group===g)&&JSON.stringify(d).toLowerCase().includes(q)).map(d=>`<article class="card diagnosis-card"><span class="tag">${esc(d.group)}</span><span class="tag source">LO ${esc(d.lo||'mapped')}</span><h3>${esc(d.name)}</h3><p><b>Also:</b> ${rich(d.also)}</p><p>${rich(d.description)}</p><p><b>Clinical:</b> ${rich(d.clinical)}</p><p><b>Testing:</b> ${rich(d.testing)}</p><p><b>Treatment:</b> ${rich(d.treatment)}</p></article>`).join('');}
...
function renderTopic(id){const m=DATA.modules.find(x=>x.id===id);let body='';
  if(id==='infection')body=diagnosisCoverage('infection')+diseaseMini(['Osteomyelitis','Septic arthritis','Disseminated gonococcal infection','Disseminated gonococcal arthritis'])+flashRows(['Osteomyelitis','Septic','Gonococcal'])+sortRows(0)+sequenceRows(0);
  else if(id==='injury')body=diagnosisCoverage('injury')+diseaseMini(['Ankle sprain','Acute knee injuries','Acromioclavicular joint separation','Muscle strain'])+flashRows(['Sprain','Strain','PRICE','Grade','Acromioclavicular'])+regionalRows(['Ankle']);
  else if(id==='genetics')body=diagnosisCoverage('genetics')+flashRows(['Duchenne','Becker','Myotonic','Osteogenesis','Gower','Ehlers','Marfan'])+diseaseMini(['Osteogenesis imperfecta','Duchenne muscular dystrophy','Becker muscular dystrophy','Myotonic dystrophy','Marfan syndrome','Ehlers-Danlos syndrome']);
  else if(id==='tumors')body=diagnosisCoverage('tumors')+imageAtlas(['tumors'])+flashRows(['Osteochondroma','Enchondroma','GCT','Osteoid','ABC','Aneurysmal'])+diseaseMini(['Osteosarcoma','Ewing sarcoma','Chondrosarcoma','Bone metastases','Osteochondroma','Enchondroma','Giant cell tumor of bone','Osteoid osteoma','Aneurysmal bone cyst'])+sortRows(1)+sortRows(4)+sequenceRows(1)+sequenceRows(2);
  else if(id==='muscle')body=diagnosisCoverage('muscle')+imageAtlas(['muscle','myopathy'])+flashRows(['T-tubule','SERCA','Cross-bridge','Myalgia','Rhabdomyolysis','NMS','Malignant','Dermatomyositis','Inclusion','Polymyalgia'])+diseaseMini(['Rhabdomyolysis','Mitochondrial myopathy','Glycogen storage myopathy','Fibromyalgia','Neuroleptic malignant syndrome','Malignant hyperthermia','Polymyalgia rheumatica','Polymyositis','Dermatomyositis','Inclusion body myositis'])+sortRows(2)+`<article class="card"><h3>Active prompt</h3><p>For muscle pain or weakness, first decide whether the patient has pain-predominant myalgia, true muscle fiber weakness, acute myonecrosis, metabolic energy failure, inflammatory myopathy, or a drug-triggered hyperthermic syndrome. CK level, urine color, medication/anesthesia exposure, rash, distribution, and tempo are the fastest discriminators.</p>${sourceMeta(DATA.sources.muscle)}</article>`;
  else if(id==='ultrasound')body=diagnosisCoverage('ultrasound')+imageAtlas(['ultrasound'])+flashRows(['Ultrasound','anisotropy','edema','collection','mass','effusion','tendon','foreign','venous'])+diseaseMini(['Soft tissue edema','Fluid collection','Solid mass','Joint effusion','Tendon tear','Foreign body','Venous thrombosis'])+`<article class="card"><h3>Dynamic assessment check</h3><p>${rich('When scanning tendon or ligament, maintain probe angle to avoid ultrasound anisotropy and use dynamic assessment when motion changes the suspected lesion.')}</p>${sourceMeta(DATA.sources.ultrasound)}</article>`;
  else if(id==='regional')body=diagnosisCoverage('regional')+caseSimulator()+imageAtlas(['knee','regional','upper','redhot'])+regionalRows()+sortRows(3)+diseaseMini(['Knee osteoarthritis','ACL tear','PCL tear','Meniscal tear','Patellofemoral pain syndrome','Prepatellar bursitis','Osgood-Schlatter disease','Baker cyst','Iliotibial band syndrome','Monoarticular arthritis','Axial spondyloarthritis','Scoliosis','Kyphosis','Spondylolysis','Degenerative disk disease','Cauda equina syndrome/cord compression','Intervertebral disc herniation','Spinal stenosis','Discitis','Musculoskeletal low back pain','Ankle sprain','Plantar fasciitis','Tarsal tunnel syndrome','Achilles tendonitis','Hallux deformity','Developmental dysplasia of the hip','Legg-Calve-Perthes disease','Slipped capital femoral epiphysis','Trochanteric bursitis','Osteonecrosis of the hip','Meralgia paresthetica','Medial/lateral epicondylitis','Scaphoid fracture','Rotator cuff tear','Little League shoulder','Dupuytren contracture']);
  renderShell(id,m.title,m.desc,m.sources,body);if(id==='regional')drawCaseSim(0);}
function diagnosisCoverage(moduleId){const rows=(DATA.diagnosis_coverage||[]).filter(x=>x.module===moduleId);if(!rows.length)return'';return `<article class="card"><div class="arena-top"><div><span class="tag">Objective alignment</span><h3>Diagnosis coverage audit</h3></div><span class="tag source">${rows.length} diagnoses</span></div><p class="small">Coverage is scaled to explicit learning objectives and exam emphasis. Each row shows the intended scope, where the diagnosis is represented in the platform, and the study focus so high-yield LO targets are not underweighted.</p><table class="table"><thead><tr><th>Diagnosis</th><th>LO scope</th><th>Platform coverage</th><th>Study focus</th></tr></thead><tbody>${rows.map(r=>`<tr><td><b>${rich(r.diagnosis)}</b></td><td><span class="tag">${esc(String(r.scope).replace(/Source-level/g,'Supplemental'))}</span><br>${rich(r.objective)}</td><td>${rich(r.coverage)}</td><td>${rich(r.focus)}</td></tr>`).join('')}</tbody></table></article>`}
...
function diseaseMini(names){return `<div class="grid">${DATA.diseases.filter(d=>names.includes(d.name)).map(d=>`<article class="card disease-mini"><span class="tag">${esc(d.group)}</span><h3>${esc(d.name)}</h3>${d.also?`<p class="small"><b>Also:</b> ${rich(d.also)}</p>`:''}<p>${rich(d.description)}</p><p><b>Clinical:</b> ${rich(d.clinical)}</p>${d.testing?`<p><b>Testing:</b> ${rich(d.testing)}</p>`:''}${d.treatment?`<p><b>Treatment:</b> ${rich(d.treatment)}</p>`:''}${sourceMeta(d.source)}</article>`).join('')}</div>`}
...
function drawHotspot(d=0){const deck=hotspotDeck();if(!document.getElementById('hotspotGame'))return;if(!deck.length){hotspotGame.innerHTML='<article class="card"><p>No hotspot rounds are available.</p></article>';return;}state.hotspotIndex=(state.hotspotIndex+d+deck.length)%deck.length;save();const idx=state.hotspotIndex,h=deck[idx],img=h.image,itemId=`hotspots:${h.index}`;hotspotGame.innerHTML=`<article class="card question-box hotspot-card">${qnav('hotspots',idx,deck.length,'drawHotspot')}<div class="image-quiz"><div><div class="hotspot-stage" data-item="${itemId}" data-x="${h.box.x}" data-y="${h.box.y}" data-w="${h.box.w}" data-h="${h.box.h}" data-tries="0" data-locked="0" onclick="checkHotspot(event)"><img src="${esc(img.data)}" alt="${esc(h.title)}" loading="lazy"><span class="hotspot-reticle" style="left:${h.box.x+h.box.w/2}%;top:${h.box.y+h.box.h/2}%"></span></div><p class="small">${esc(img.mode||'Image round')}</p>${sourceMeta(img.source)}</div><div><div class="meta"><span class="tag">Hotspot</span><span class="tag source">${esc(h.key)}</span></div><h3>${esc(h.title)}</h3><p>${rich(h.prompt)}</p><p class="small"><b>Target:</b> ${esc(h.target)}. Click directly on the image.</p>${confidenceBar(itemId)}<div id="hotspotFeed" class="feedback"></div></div></div></article>`;}
...
function renderVisuals(){const m=DATA.modules.find(x=>x.id==='visuals');const body=`<div class="activity-tabs"><button class="primary" onclick="drawImageQuiz(0)">Quiz current image</button><button onclick="drawImageQuiz(1)">Next image</button></div><div id="imageQuiz"></div><article class="card"><h3>Full visual atlas</h3>${imageAtlas()}</article>`;renderShell('visuals',m.title,m.desc,m.sources,body);drawImageQuiz(0);}
function drawImageQuiz(d=0){const deck=imageDeck();if(!document.getElementById('imageQuiz'))return;if(!deck.length){imageQuiz.innerHTML='<article class="card"><p>No image quiz rounds are available.</p></article>';return;}state.imageIndex=(state.imageIndex+d+deck.length)%deck.length;save();const idx=state.imageIndex,img=deck[idx];imageQuiz.innerHTML=`<article class="card question-box"><div class="image-quiz"><div>${qnav('visuals',idx,deck.length,'drawImageQuiz')}${zoomableImage(img.data,img.title+' - '+img.source)}<p class="small">${esc(img.mode||'Image round')}</p>${sourceMeta(img.source)}</div><div><div class="meta"><span class="tag">Image round</span><span class="tag source">${esc(img.key)}</span></div><h3>${esc(img.title)}</h3><p>${rich(img.prompt||'Choose the best visual diagnosis.')}</p><div class="visual-options" data-wrong="0" data-total="${img.choices.length}" data-locked="0">${img.choices.map(o=>`<button class="choice" data-value="${esc(o)}" data-answer="${esc(img.answer)}" onclick="checkImageChoice(this,'visuals:${idx}')">${rich(o)}</button>`).join('')}</div><div id="imageFeed" class="feedback"></div></div></div></article>`;}
...
function imageAtlas(keys){let imgs=DATA.images;if(keys)imgs=imgs.filter(img=>keys.includes(img.key));return `<div class="grid">${imgs.map((img,i)=>`<article class="card"><span class="tag">${esc(img.mode||'Image prompt')}</span><h3>${esc(img.title)}</h3>${zoomableImage(img.data,img.title+' - '+img.source)}${sourceMeta(img.source)}</article>`).join('')}</div>`}
...
function cleanUiText(s){
  return String(s??'')
    .replace(/source-scoped/gi,'interactive')
    .replace(/source-aligned/gi,'exam-style')
    .replace(/source-mapped/gi,'mapped')
    .replace(/source image prompts/gi,'image prompts')
    .replace(/source image feature/gi,'image feature')
    .replace(/source image/gi,'image');
}
...
function moduleRail(id){
  ...
  const items=[['Top','scrollTo(0,0)'],['Home',`showSection('dashboard')`],['OSCE',`showSection('osce')`],['Images',`showSection('visuals')`],['Anatomy',`showSection('anatomy')`],['Compare',`showSection('discriminators')`],['Repair',`showSection('repair')`],['LO map',`showSection('objectives')`]];
...
function globalSearch(query){
  ...
  Object.values(DATA.osce_physical_exam?.regions||{}).forEach(region=>{
    (region.checklist||[]).forEach(item=>addSearchResult(results,q,'OSCE checklist',`${region.label}: ${item.item}`,`${item.category} ${item.steps}`,'osce',6));
    (region.map||[]).forEach(item=>addSearchResult(results,q,'OSCE finding map',`${region.label}: ${item.exam}`,`${(item.diagnoses||[]).join(' ')} ${item.finding}`,'osce',6));
  });
  (DATA.osce_physical_exam?.cases||[]).forEach(item=>addSearchResult(results,q,'OSCE diagnosis case',item.title,`${item.region} ${item.diagnosis} ${item.alternative} ${item.history} ${(item.examFindings||[]).join(' ')} ${(item.evidence||[]).join(' ')}`,'osce',7));
...
function renderDashboard(){
  ...
  const cards=[{id:'objectives',title:'Learning Objective Tracker',tag:'Reference',desc:`${DATA.objectives.length} validated objectives from ${DATA.objective_source||'learning objective spreadsheet'}.`},{id:'glossary',title:'Glossary',tag:'Reference',desc:'Alphabetized terms with term sprint practice.'},{id:'disease',title:'Disease / Diagnosis Index',tag:'Reference',desc:'Searchable disease and diagnosis cards with compact high-yield summaries.'},{id:'drugs',title:'Drug and Mechanism Matrix',tag:'Reference',desc:'Antibiotic, chemotherapy, and osteoprotective medication anchors.'},...DATA.modules.map(m=>({...m,tag:['visuals','hotspots','discriminators','management','repair','osce'].includes(m.id)?'Practice mode':'Practice',desc:`${m.desc} ${loCountForModule(m)} mapped LOs.`}))];
  const body=q?searchPanel(q):`<div class="grid">${cards.map(m=>`<article class="card module-card"><div><span class="tag">${esc(m.tag)}</span><h2>${esc(m.title)}</h2><p>${rich(cleanUiText(m.desc))}</p></div><div class="small">${Object.keys(state.completed).filter(k=>k.startsWith(m.id+':')).length} completed</div><button class="primary" onclick="showSection('${m.id}')">Open</button></article>`).join('')}</div>`;
...
function ensureSectionRendered(id){
  ...
  else if(id==='osce')renderPhysicalExam();
...
function imageAttribution(img){return '';}

function imageAtlas(keys){
  let imgs=DATA.images;
  if(keys)imgs=imgs.filter(img=>keys.includes(img.key));
  return `<div class="grid">${imgs.map((img,i)=>`<article class="card"><span class="tag">${esc(img.mode||'Image prompt')}</span><h3>${esc(img.title)}</h3>${zoomableImage(img.data,img.title+' - '+img.source)}${sourceMeta(img.source)}${imageAttribution(img)}</article>`).join('')}</div>`;
}
...
function renderVisuals(){
  ...
  const body=`<div class="activity-tabs"><button class="primary" onclick="drawImageQuiz(0)">Quiz current image</button><button onclick="drawImageQuiz(1)">Next image</button></div><div id="imageQuiz"></div><details class="card"><summary><b>Full visual atlas</b> - load only when you need the full image bank</summary><div id="visualAtlasMount" class="atlas-placeholder"><p>Atlas images are deferred so the quiz loads fast and hidden images do not inflate the page.</p><button onclick="loadVisualAtlas()">Load visual atlas</button></div></details>`;
...
function mixedSourceDeck(){
  ...
  DATA.management.forEach((m,i)=>items.push(normalizedQuestion({module:m.module,lo:m.lo||'mapped',skill:'Management',source:m.source,stem:m.stem,question:m.question,choices:m.choices,answer:m.answer,why:m.why,clue:m.urgency||'Choose the next best action.',distractors:{}},`management:${i}`,'Management')));
  DATA.discriminators.forEach((d,i)=>items.push(normalizedQuestion({module:d.module,lo:d.lo||'mapped',skill:'Discriminator',source:d.source,stem:d.stem,question:d.question,choices:d.choices,answer:d.answer,why:d.why,clue:d.pair,distractors:{}},`discriminator:${i}`,'Discriminator')));
...
function drawQuestionDeck(id,d=0){
  ...
  renderShell(id,id==='clinical'?'Clinical Vignettes':'Boss Round Mixed Review',id==='clinical'?'Original exam-style questions with full explanations.':'Interleaved review across clinical vignettes, LO drills, management, discriminators, and regional cases.',[q.source],`<article class="card question-box">${qnav(id,idx,deck.length,`drawQuestionDeck('${id}')`)}<div class="meta"><span class="tag source">${esc(q.module)}</span><span class="tag source">LO ${esc(q.lo||'mapped')}</span><span class="tag">${esc(q.kind||q.skill)}</span></div><p><b>${rich(q.stem)}</b></p><p>${rich(q.question)}</p><div data-wrong="0" data-total="${q.choices.length}" data-locked="0">${q.choices.map(o=>`<button class="choice" onclick="guarded(this,'${esc(o)}','${esc(q.answer)}','${esc(q.why)}','${itemId}','qfeed',${esc(JSON.stringify(q.distractors||{}))},'${esc(q.clue||'High-yield discriminator.')}')">${rich(o)}</button>`).join('')}</div><button onclick="state.flagged['${itemId}']=!state.flagged['${itemId}'];save();this.textContent=state.flagged['${itemId}']?'Flagged':'Flag question'">${state.flagged[itemId]?'Flagged':'Flag question'}</button><div id="qfeed" class="feedback"></div></article>`);
  showSection(id);
}
