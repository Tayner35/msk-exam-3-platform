...
function diagnosisCoverage(moduleId){const rows=(DATA.diagnosis_coverage||[]).filter(x=>x.module===moduleId);if(!rows.length)return'';return `<article class="card"><div class="arena-top"><div><span class="tag">Objective alignment</span><h3>Diagnosis coverage audit</h3></div><span class="tag source">${rows.length} diagnoses</span></div><p class="small">Coverage is scaled to explicit learning objectives and source emphasis. Each row shows the intended scope, where the diagnosis is represented in the platform, and the study focus so high-yield LO targets are not underweighted.</p><table class="table"><thead><tr><th>Diagnosis</th><th>LO scope</th><th>Platform coverage</th><th>Study focus</th></tr></thead><tbody>${rows.map(r=>`<tr><td><b>${rich(r.diagnosis)}</b></td><td><span class="tag">${esc(r.scope)}</span><br>${rich(r.objective)}</td><td>${rich(r.coverage)}</td><td>${rich(r.focus)}</td></tr>`).join('')}</tbody></table></article>`}
